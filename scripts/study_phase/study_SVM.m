% ====================================================================
% Multivariate SVM Classifier: STUDY phase (Remembered vs Forgotten)
% ====================================================================
...
if useRawPath
    ...
    outputFileName = 'SVM_study_results_raw.mat';
    fprintf('Using STUDY RAW data path\n');
else
    % STUDY FILTERED features (update this to your filtered-study folder)
    ...
    outputFileName = 'SVM_study_results_filtered.mat';
    fprintf('Using STUDY FILTERED data path\n');
end

% Output directory (same for both)
outputDir = "C:\Users\Angad\OneDrive\Desktop\Comp Memory Lab\Classifier.2\outputs\SVM_study";
...
for i = 1:length(featureFiles)
    ...
    data = load(file);  % expect X, y, params (y: 0 = forgotten, 1 = remembered)
    ...
    % Extract participant ID from filename (digits before .mat)
    ...
    % Sanity check: need both remembered (1) and forgotten (0)
    if numel(unique(y)) < 2
    ...
    % We treat the whole participant as a separate dataset and run SVM
    % with internal CV. No participant IDs needed inside the function.
    
    % Pass [] for participant, and a label for logging
    AUC_all.(subjField).StudyMem = study_run_svm_auc(X, y, [], 'StudyMem');
    
    fprintf('Completed STUDY classification for %s (AUC = %.3f)\n', ...
        subjField, AUC_all.(subjField).StudyMem);
end

% ------------------ Save Results ------------------
save(fullfile(outputDir, outputFileName), 'AUC_all');
fprintf('\nSTUDY-phase SVM classification completed and saved as %s.\n', outputFileName);
